...
describe('Warning groups in Debug Journey', () => {
    ...
    it('uses one shared grouping for lifecycles, statistics and notebook export', async () => {
        const event = compileError();
        // Continuous journeys retain both sign-compare sites in one warning lifecycle.
        const lifecycles = buildErrorLifecycles([event]);
        assert.strictEqual(lifecycles.length, 4);
        assert.strictEqual(lifecycles.find(l => l.warning?.diagnostic.code === '-Wsign-compare')?.warning?.locations.length, 2);
        ...
        assert.strictEqual(notebook.summary.lifecycles.length, 4);
    ...
    it('keeps the same root signature across repeated compiles and uses logical retry counts', () => {
        ...
        assert.strictEqual(reorder.length, 1);
        assert.strictEqual(reorder[0].entries.length, 2);
        assert.strictEqual(reorder[0].attemptsBeforeResolve, 1);
