			...
			...(input.referenceTargets && input.referenceTargets.length > 0
				? [{
					...
					content: [
						...
						'Multi-line code blocks report their source the same way: every fenced code block MUST end with a refblock line placed immediately after the closing fence (before any other text). It is part of the required answer format, like a citation:',
						'```cpp',
						'std::cout << "turn" << std::endl;',
						'```',
						'{{refblock:sym:monster.h:Monster:takeTurn}}',
						'List the targetIds of the symbols the block comes from (comma-separated). The program turns the line into a visible source note; never write classmate-ref:// links or source notes yourself.',
						'Rules:',
