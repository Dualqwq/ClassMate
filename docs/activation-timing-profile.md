...
1. **目标口径**:以"干净 profile 冷启动 p50"为准(可复现、噪声小),还是要覆盖"会话恢复"场景(121–285 ms,噪声主导)?建议前者做门禁，后者只做观察。
2. **是否接受架构改动**:为达成 ≤40 ms 需把图执行链改动态 import,触及 `ChatSession`/graph 装配代码；若不接受，目标只能定在维持现状(≤85 ms)。
3. **门禁用哪个构建形态**:建议用 prod 包(与 VSIX 一致);dev 包数字仅开发参考。
4. **探针是否固化**:当前驱动/探针/分析脚本为一次性，放 `log/` 不入库；是否要在后续优化分支里固化为可重复跑的 `scripts/`(类似 `eval:review`)?
5. **后台任务口径**:SecretStorage 读取(~33 ms)与工作区 refresh(~78 ms)是否计入"激活后可用时间"指标，还是维持 fire-and-forget 现状不考核?
