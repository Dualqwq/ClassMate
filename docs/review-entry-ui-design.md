...
> 状态:**已拍板并定稿(2026-08-29 用户)**:Q5 放宽——digest 预算放宽到 ~10000 字符,
> 且原 answer 路径的 FE2 digest 一并放宽(全局统一,不分场景);Q1-Q4/Q6-Q10 全部按
> 推荐执行(拍板结果见 §8.1)。2026-08-28 产出,worktree `docs/review-entry-ui-plan`。
...
- 预算与条数上限(2000 字符/每节 5 条)对复习是否够用:列为待拍板 Q5,
  ...
  分话题再问一次,而非加大注入。
  **拍板(2026-08-29):放宽**——用户拍板 digest 预算放宽到 ~10000 字符,且原
  answer 路径的 FE2 digest 一并放宽(全局统一,不分场景);原「信任底线」顾虑由
  用户知情选择覆盖(高频多卡学生的覆盖优先)。实施切片见 §8.1。
...
### 8.1 拍板结果(2026-08-29 用户)

- **Q5:放宽(用户推翻默认)**——digest 预算放宽到 **~10000 字符**,且原 answer 路径的
  FE2 digest 一并放宽(**全局统一**:复习与普通回答同一预算,不做场景分支);每节条数
  上限同比例放宽(5 → 25),实施轨可按实测微调但须在报告说明。
- **其余全部按推荐执行**:Q1=A+B 两期组合、Q2=复习指令不带 intent、Q3=采用 §4.4 预填
  文案草案、Q4=期 2 队列上限 10 张、Q6=复习行为不落盘、Q7=注册 `classmate.reviewMistakes`、
  Q8=徽标再议(本轨不做)、Q9=「让 AI 带我复盘」放错题本过滤栏、Q10=切页签复位。
- **实施安排**:期 1(`feat/review-entry-chat`)与 digest 放宽(`feat/journey-digest-budget`)
  并行开工;期 2(`feat/review-queue`)待期 1 合入后开工(两切片都要改 MistakeBookTab,
  串行避免同文件冲突)。
